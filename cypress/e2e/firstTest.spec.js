...
describe('First test suite', () => {
    ...
    it('save subject of the command', () => {
        ...
        // using the Cypress then() method only visible inside its own function 
    ...
    it('Web Tables', () => {
        ...
        cy.get('tbody').contains('tr', 'Larry').then(tableRow => {
            ...
            // finding by index of the tables, no unique selector, sixth column in the table row
   ...
   // get each row for validation and checking for differant parameters 
            const age = [20, 30, 40, 200]
               cy.wrap(age).each( age => {
                cy.get('thead [plaholder="Age"]').clear().type(age)
                cy.wait(500)
                cy.get('tbody tr').each( tableRow => {
                    if(age == 200){
                        cy.wrap(tableRow).should('contain', 'No date found')
                    } else {
                        cy.wrap(tableRow).find('tb').eq(6).should('contain', age)
                    }        
                })  
            })
        })
